...
export default function PackagesSection() {
  // Separate scroll controls for each row
  const { containerRef: topContainerRef, scroll: scrollTop } = useScrollContainer();

  const { containerRef: bottomContainerRef, scroll: scrollBottom } = useScrollContainer();

  // Split packages into two rows
  const middleIndex = Math.ceil(packages.length / 2);

  const topRowPackages = packages.slice(0, middleIndex);
  const bottomRowPackages = packages.slice(middleIndex);

  // Move both rows when using the arrow buttons
    ...
    <section id="packages" className="pt-12 pb-12 md:pt-20 md:pb-20 bg-white">
      <ScrollReveal className="container-custom">
        ...
        <div className="relative">
          {/* Arrows */}
          <div className="flex justify-end gap-2 sm:gap-3 mb-4 md:mb-6">
            <button
          ...
          >
            {topRowPackages.map((pkg) => (
              <div
                key={pkg.id}
                className="
                  shrink-0
                  w-[calc(100%-96px)]
                  sm:w-[calc((100%-96px)/2)]
                  lg:w-[calc((100%-112px)/3)]
                "
              >
                <PackageCard pkg={pkg} />
              </div>
